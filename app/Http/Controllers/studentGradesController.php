...
class studentGradesController extends Controller
{
    ...
    public function ranks(Request $request){
    	...
    	if(isset($request->classid)){
            ...
            if($state=="overall"){
                $data['students'] = Student::select(DB::raw('SUM(test_score)/COUNT(distinct test_academics.test_name) * 0.3 + SUM(exam_score) as score'), 'students.id as studentid', 'first_name', 'last_name', 'test_academics.term', 'students.class')
                        ->join('classes','students.class','=','classes.class_name')
                        // ->join('class_subjects','classes.id','=','class_subjects.class_id')
                        ->leftJoin('test_academics','students.id','=','test_academics.student_id')
                        ->leftJoin('exam_academics',function($join){
                        ...
                        ->where($where)
                        ->groupBy('students.id','last_name','first_name','class','test_academics.term')   
                        ->orderBy('score','DESC')
                        ->get(); 
            }
    		
            else {
                $data['students'] = Student::select(DB::raw('SUM(test_score)/COUNT(distinct test_academics.test_name) * 0.3 + SUM(exam_score) as score'), 'students.id as studentid', 'first_name', 'last_name', 'test_academics.term', 'subjects.subject_name as class')
                        ->join('classes','students.class','=','classes.class_name')
                        ->leftJoin('test_academics','students.id','=','test_academics.student_id')
                        ->leftJoin('exam_academics',function($join){
                            $join->on('students.id','=','exam_academics.student_id')
                        ...
                        ->join('subjects','subjects.id','=','exam_academics.subject_id')
                        ->where($where)
                        ->groupBy('students.id','last_name','first_name','test_academics.term','subjects.subject_name')      
                        ->orderBy('score','DESC')
                        ->get();
            }
...
// select sum(test_score)/COUNT(distinct test_name) + sum(exam_score) as total_test_score, students.id, first_name, last_name, test_academics.term, students.class from students left join test_academics on students.id = test_academics.student_id left join exam_academics on students.id = exam_academics.student_id and test_academics.subject_id = exam_academics.subject_id inner join classes on students.class = classes.class_name group by students.id, first_name, last_name, test_academics.term, students.class 
